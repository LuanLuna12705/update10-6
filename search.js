document.addEventListener("DOMContentLoaded", function () {
  ...
  if (savedFilters) {
    ...
    if (location && savedFilters.location) location.value = savedFilters.location;
  ...
  if (locationFilter || priceFilter) {
    ...
    items.forEach(item => {
      const address = item.querySelector(".main__info--add1")?.innerText.toLowerCase();
      ...
      let matchLocation = !locationFilter || (address && normalize(address).includes(locationNorm) || normalize(address).includes(locationNorm + " ha noi"));
