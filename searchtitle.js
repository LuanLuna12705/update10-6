...
const listings = Array.from(document.querySelectorAll(".main__item")).map(item => {
  return {
    element: item,
...
function renderSuggestions(results) {
  ...
  results.forEach(result => {
    ...
    div.addEventListener("click", () => {
      ...
      document.getElementById("location").value = normalizeString(result.location);
...
searchInput.addEventListener("input", (e) => {
  ...
  const matched = listings.filter(item => normalizeString(item.code).includes(query));
